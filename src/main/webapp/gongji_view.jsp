...
<table>
	...
	<td>
	<input type=button onClick="location.href='gongji_delete.jsp?key=<%=id%>&rootid=<%=rootid %>&relevel=<%=relevel%>'" value = '삭제'> </td>
		<td>
	<button type="button" onclick="location.href='gongji_reinsert.jsp?key=<%=id%>&relevel=<%=relevel%>&recnt=<%=recnt%>'">댓글</button></td>
	</tr>
</table>
<%
	}
...
<table border = 1 cellpadding=5 id="bigtable">
	...
	<td width=480 colspan=3 align=left><input type='hidden' name="rootid" value=<%=rootid%>><%=rootid%></td>
		...
		<tr>
	<td width=150>댓글 내 순서 </td>
	<td width=150 align=left><input type='hidden' name="recnt" value=<%=recnt%>><%=recnt%></td>

	<td width=150 align=left>댓글수준<input type='hidden' name="relevel" value=<%=relevel%>></td>
	<td width=150 align=left><%=relevel %></td>
	</tr>
</table>
<br>
<table>
	...
	<input type=button onClick="location.href='gongji_update.jsp?key=<%=id%>'" value = '수정'> </td>
	<td>
	<input type=button onClick="location.href='gongji_delete.jsp?key=<%=id%>&rootid=<%=rootid %>&relevel=<%=relevel%>&recnt=<%=recnt%>'" value = '삭제'> </td>
		<td>
	<button type="button" onclick="location.href='gongji_reinsert.jsp?key=<%=id%>&relevel=<%=relevel%>&recnt=<%=recnt%>'">댓글</button></td>
	</tr>
</table>
